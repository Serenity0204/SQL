...
bool BPlusTree<Item>::insert(const Item& entry)
{
    ...
    if (this->data_count > MAXIMUM)
    {
        ...
        copy_array(new_root->data, this->data, new_root->data_count, this->data_count);
        copy_array(new_root->subset, this->subset, new_root->child_count, this->child_count);
...
bool BPlusTree<Item>::loose_insert(const Item& entry)
{
    ...
    if (this->is_leaf() && !found)
    {
        insert_item(this->data, index, this->data_count, entry);
        return true;
    }
    if (!this->is_leaf() && found)
    {
...
void BPlusTree<Item>::fix_excess(std::size_t i)
{
    ...
    split(subtree->data, subtree->data_count, split_tree->data, split_tree->data_count);
    split(subtree->subset, subtree->child_count, split_tree->subset, split_tree->child_count);
    Item item;
    detach_item(subtree->data, subtree->data_count, item);
    ...
    insert_item(this->subset, i + 1, this->child_count, split_tree);
}
...
bool BPlusTree<Item>::loose_erase(const Item& entry)
{
    ...
    {
        Item item;
        delete_item(this->data, index, this->data_count, item);
        return true;
    }
...
BPlusTree<Item>* BPlusTree<Item>::merge_with_next_subset(std::size_t i)
{
    ...
    if (!left_subset->is_leaf())
        attach_item(left_subset->data, left_subset->data_count, item);
    // merge
